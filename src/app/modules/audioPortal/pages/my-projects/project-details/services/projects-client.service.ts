...
export class ProjectsClientService {
   ...
   */
  reorderChapters(projectId: number, orderedIds: number[]): Observable<ApiResponse<null>> {
    const url = this.api.chapters.reorder(projectId);
    const body = { ordered_ids: orderedIds };
    return this.http.action(url, body, 'reorderChapters');
  }

  /**
   * Reorder paragraphs (future implementation)
   */
  reorderParagraphs(chapterId: number, orderedIds: number[]) {
    const url = this.apiPortal.paragraphs.reorder(chapterId);
    return this.httpService.action(url, { ordered_ids: orderedIds }, 'reorderParagraphs');
  }
  ...
  generateVoice(request: GenerateVoiceRequest): Observable<number> {
    ...
    switch (request.type) {
      case VoiceEntityType.Project:
        url = this.apiPortal.voices.generateProject;
        break;
      case VoiceEntityType.Chapter:
        url = this.apiPortal.voices.generateChapter;
        break;
      case VoiceEntityType.Paragraph:
        url = this.apiPortal.voices.generateParagraph;
        break;
      default:
        return throwError(() => new Error('Invalid voice entity type'));
    }
    ...
    return this.httpService.list(url, request, 'generateVoice').pipe(
      ...
      catchError(err => {
        console.error('[ProjectsClientService] Voice generation failed:', err);
