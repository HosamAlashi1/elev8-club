...
export class AddEditAuthorComponent implements OnInit {
  ...
  @Input() author: any; // في حالة التعديل
  form!: FormGroup;
  submitted = false;
  ...
  ngOnInit() {
    ...
    if (this.isEdit) {
      this.httpService.listGet(this.api.users.details(this.author.id), 'user-details').subscribe({
        next: (res: any) => {
          if (res?.status && res?.data) {
            this.patchForm(res.data);
          } else {
            this.toastr.showError(res?.message || 'Failed to load author');
          }
        },
        error: () => this.toastr.showError('Failed to load author')
      });
    }
  }
  ...
  initForm() {
    this.form = new FormGroup({
      first_name: new FormControl(this.author?.first_name || '', [Validators.required, Validators.maxLength(190)]),
      middle_name: new FormControl(this.author?.middle_name || '', [Validators.maxLength(190)]),
      last_name: new FormControl(this.author?.last_name || '', [Validators.required, Validators.maxLength(190)]),
      email: new FormControl(this.author?.email || '', [Validators.required, Validators.email, Validators.maxLength(190)]),
      phone: new FormControl(this.author?.phone || '', [Validators.required, Validators.maxLength(30)]),
  ...
  submit() {
    ...
    formData.append('auth_type', '2'); // دايمًا 2 للمؤلفين
    ...
    const url = this.isEdit
      ? this.api.users.edit(this.author.id)
      : this.api.users.add;

    this.httpService.action(url, formData, 'addEditUser').subscribe({
      next: (res: any) => {
        if (res?.status) {
          this.toastr.showSuccess(res?.message || 'Author saved successfully');
