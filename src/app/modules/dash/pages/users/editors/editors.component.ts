...
import { AddEditEditorComponent } from './add-edit/add-edit.component';
...
export class EditorsComponent implements OnInit {
  ...
  buildUrl(): string {
    ...
    let url = `${this.api.users.list}?q=${q}&auth_type=3&size=${this.size}&page=${this.page}`;
    
    if (this.statusFilter) {
      const statusValue = this.statusFilter === 'active' ? '1' : '0';
  ...
  openImageModal(image: string) {
    this.publicService.openImage('Editor', image);
  }
  ...
  add() {
    const modalRef = this.modalService.open(AddEditEditorComponent, { size: 'md', centered: true });
    modalRef.result.then(() => this.list(1));
  }

  edit(item: any) {
    const modalRef = this.modalService.open(AddEditEditorComponent, { size: 'md', centered: true });
    modalRef.componentInstance.editor = item;
    modalRef.result.then(() => this.list(1));
  }
  ...
  delete(item: any) {
    ...
    modalRef.componentInstance.type = 'editor';
    modalRef.componentInstance.message = `Do you want to delete ${this.fullName(item)} editor ?`;
