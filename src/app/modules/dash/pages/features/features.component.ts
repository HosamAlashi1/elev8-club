...
export class FeaturesComponent implements OnInit {
  ...
  list(page: number): void {
    ...
    const url = `${this.api.features.list}`;
    this.httpService.list(url, payload, 'featuresList').subscribe({
      next: (res) => {
      ...
      error: () => {
        this.toastr.Showerror('Failed to load features');
        this.isLoading$.next(false);
      }
    });
  }
  ...
  edit(item: any) {
    const modalRef = this.modalService.open(AddEditComponent, { size: 'lg', centered: true });
    modalRef.componentInstance.feature = item;
    modalRef.result.then(() => this.list(1));
  }
  ...
  delete(item: any) {
    ...
    modalRef.componentInstance.type = 'features';
    modalRef.componentInstance.message = `Do you want to delete "${item.title_en}" feature?`;
    modalRef.result.then(() => this.list(1));
  }
