...
export class PackagesComponent implements OnInit {
    ...
    this.httpService.list(url, payload, 'packagesList').subscribe({
      next: (res) => {
        if (res?.status && res?.data?.data) {
          this.packages = res.data.data.map((p: any) => ({
            ...p,
            image: p.image || 'assets/img/blank.png'
          }));
      ...
      error: () => {
        this.toastr.showError('Failed to load packages');
        this.isLoading$.next(false);
      }
    });
  }
  ...
  delete(item: any) {
    ...
    modalRef.componentInstance.type = 'package';
    modalRef.componentInstance.message = `Do you want to delete ${item.name} ?`;
    modalRef.result.then(() => this.list(1));
  }

  openImageModal(image: string) {
    this.publicService.openImage('Package Image', image);
  }
}
